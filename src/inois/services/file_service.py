...
class FileService:
    ...
    def validate_files(cls, config):
        ...
        cls.validate_files_are_csv(config)
    ...
    def navigate_to_working_directory(config):
        ...
        if not os.path.exists(config.WORKING_DIRECTORY):
            logging.error(Notifications.DIRECTORY_NOT_FOUND_ERROR.format(os.path.join(config.WORKING_DIRECTORY)))
    ...
    @staticmethod
    def validate_files_are_csv(config):
